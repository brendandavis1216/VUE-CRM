...
import * as z from "zod"; // Corrected this line
...
export const InquiryForm: React.FC<InquiryFormProps> = ({ onSubmit, onClose, defaultValues }) => {
  ...
  const [isPowerProvidedChecked, setIsPowerProvidedChecked] = useState(powerValue === "Provided");
  ...
  function handleSubmit(values: InquiryFormValues) {
    onSubmit(values);
    form.reset();
    setIsPowerProvidedChecked(false); // Reset checkbox state
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
        <div className="max-h-[calc(100vh-12rem)] overflow-y-auto p-2"> {/* Added scrollable div */}
          <FormField
        ...
        <Button type="submit" className="w-full bg-primary text-primary-foreground hover:bg-primary/90 mt-4">Add Inquiry</Button>
      </form>
    </Form>
  );
};